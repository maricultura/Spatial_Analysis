...
extent_zoomed <- extent(c(6647998, 7500683, 8271044, 9114753))

# Crop suitability map to zoomed area
zoomed_all_layers <- crop(all_layers, extent_zoomed)
...
# Cropping boundaries of Brazil
BRA_cropped <- st_crop(bound_BRA, extent_zoomed)
...
Biomass Graph
```{r}
biomass_raster <- raster("/Volumes/GoogleDrive/Shared drives/Bren GP 2020 maricultura/Data/Raw_Data/Map_Outputs/biomass_map.tif")
...
biomass_graph <- ggplot() +
    geom_raster(data = biomass_df,
                mapping = aes(x = x, y = y, fill = biomass_map)) +
   scale_fill_gradient(
   low = "#EEE35F", high = "#1F9E10", na.value = "transparent", name = "Biomass (kg/cell)") +
    geom_sf(data = BRA_cropped, fill = "#DCDBE0") +
    geom_sf(data = bound_BRA, fill = "#DCDBE0") +
   theme_void() +
  theme(panel.grid.major = element_line(color = "white"))
...
cropped_biomass <- crop(biomass_raster, extent_zoomed)


cropped_biomass_df <- as.data.frame(cropped_biomass, xy = TRUE)

#Cropping EEZ to put in background
eez_all_1_cropped <- crop(eez_all_1, extent_zoomed)

# Make df
eez_all_1_cropped_df <- as.data.frame(eez_all_1_cropped, xy = T)

# Crop polygon EEZ
eez_BRA_cropped <- st_crop(eez_BRA, extent_zoomed)
...
npv_raster <- raster("/Volumes/GoogleDrive/Shared drives/Bren GP 2020 maricultura/Data/Raw_Data/Map_Outputs/npv.tif")

npv_df <- as.data.frame(npv_raster, xy = TRUE)

npv_graph <- ggplot() +
    geom_raster(data = npv_df,
                mapping = aes(x = x, y = y, fill = npv)) +
   scale_fill_gradient(
...
cropped_npv <- crop(npv_raster, extent_zoomed)
